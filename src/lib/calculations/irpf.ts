// Modelo simplificado de referencia: no aplica escalas ni mínimos autonómicos.
export interface IrpfInput { salary: number; otherIncome: number; savingsIncome: number; socialContributions: number; pensionContributions: number; withheld: number; age: number; children: number; childrenUnder3: number; dependantsOver65: number; disability: 'none' | '33' | '65'; filing: 'individual' | 'joint'; }
...
export function calculateIrpf(input: IrpfInput): IrpfResult {
  ...
  if (!Number.isInteger(input.dependantsOver65) || input.dependantsOver65 < 0) throw new Error('Introduce un número entero de ascendientes igual o mayor que cero.');
